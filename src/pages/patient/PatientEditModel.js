...
const PatientEditModal = ({ isOpen, onClose, patient, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    if (
      !editedPatient.name ||
      !editedPatient.phone ||
      !editedPatient.birth_date ||
      !editedPatient.id_card
    ) {
    ...
    try {
      const response = await fetch(
        `https://clinic-app-be.onrender.com/api/patients/${patient._id}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <Modal isOpen={isOpen} toggle={onClose} size="lg">
      <ModalHeader toggle={onClose}>Chỉnh Sửa Thông Tin Bệnh Nhân</ModalHeader>
      <ModalBody>
        <Form>
          <FormGroup row>
